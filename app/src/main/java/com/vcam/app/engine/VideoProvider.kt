...
class VideoProvider : ContentProvider() {
    companion object { private const val TAG = "VCam-Provider" }
    ...
    override fun openFile(uri: Uri, mode: String): ParcelFileDescriptor? {
        return try {
            ...
            val videoPath = prefs.getString("video_path_final", "") ?: ""
            val imagePath = prefs.getString("image_path_final", "") ?: ""

            val path = when {
                videoPath.isNotBlank() && File(videoPath).exists() -> videoPath
                imagePath.isNotBlank() && File(imagePath).exists() -> imagePath
                else -> ""
            }

            if (path.isBlank()) {
                Log.w(TAG, "No video/image file found"); return null
            }

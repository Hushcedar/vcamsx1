...
object VideoPlayer {
    ...
    fun camera2Play() {
        MainHook.original_preview_Surface?.let { s ->
            if (s.isValid) {
                if (ImagePlayer.isActive) ImagePlayer.attachSurface(s)
    ...
    fun rotate() {
        ...
        if (ImagePlayer.isActive) { ImagePlayer.rotate(); return }
    ...
    fun zoomIn() {
        if (ImagePlayer.isActive) { ImagePlayer.zoomIn(); return }
        try {
    ...
    fun zoomOut() {
        if (ImagePlayer.isActive) { ImagePlayer.zoomOut(); return }
        try {

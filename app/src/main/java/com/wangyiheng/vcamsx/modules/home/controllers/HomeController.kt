...
class HomeController : ViewModel(), KoinComponent {
    ...
    val isRealSceneEnabled     = mutableStateOf(false)  // "Open The Real Scene" toggle
        ...
        mediaPlayer = IjkMediaPlayer().apply {
            try {
                ...
                setOnErrorListener { _, what, extra ->
                    Toast.makeText(context, "Stream error: $what", Toast.LENGTH_SHORT).show()
                    true
                }
                dataSource = rtmpUrl
                setDisplay(holder)
                ...
                setOnPreparedListener {
                    Toast.makeText(context, "Stream connected", Toast.LENGTH_SHORT).show()
                    start()
                }
            } catch (e: Exception) {

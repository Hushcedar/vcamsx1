...
object ImagePlayer {
    ...
    private const val TAG = "VCamImagePlayer"
    ...
    fun loadImage(context: Context, uri: Uri, onResult: (Boolean) -> Unit) {
        ...
        } catch (e: Exception) { Log.e(TAG, "read: ${e.message}"); null }

        if (bytes == null || bytes.isEmpty()) {
            mainHandler.post { isLoading.value = false; onResult(false) }
            return
        }
        Thread({ decodeAndEncode(context, bytes, onResult) }, "VCam-ImgLoad")
    ...
    private fun decodeAndEncode(context: Context, bytes: ByteArray, onResult: (Boolean) -> Unit) {
        try {
            val probe = BitmapFactory.Options().apply { inJustDecodeBounds = true }
            BitmapFactory.decodeByteArray(bytes, 0, bytes.size, probe)
            if (probe.outWidth <= 0) { mainHandler.post { isLoading.value = false; onResult(false) }; return }
            ...
            val outFile = VideoProvider.getVideoFile(context)
            Log.d(TAG, "encoding image→mp4…")
            bitmapToMp4(bmp, outFile)
            val ok = outFile.exists() && outFile.length() > 4096L
            Log.d(TAG, "encode done ok=$ok size=${outFile.length()}b")

            mainHandler.post {
                hasImage.value  = ok
                isLoading.value = false
                if (ok) isActive.value = true
                onResult(ok)
            }
        } catch (e: Exception) {
            Log.e(TAG, "decodeAndEncode: ${e.message}", e)
            mainHandler.post { isLoading.value = false; onResult(false) }
        }
    }

    /**
     * Encodes bitmap as a 10-second H.264 MP4 at 10 fps using an EGL render
     * thread pushing into the MediaCodec input surface.
     * lockCanvas() is NOT used — it crashes on codec surfaces above API 23.
     */
    private fun bitmapToMp4(src: Bitmap, outFile: File) {
        ...
        val eglContext = EGL14.eglCreateContext(eglDisplay, configs[0]!!, EGL14.EGL_NO_CONTEXT,
            intArrayOf(EGL14.EGL_CONTEXT_CLIENT_VERSION, 2, EGL14.EGL_NONE), 0)
        val eglSurface = EGL14.eglCreateWindowSurface(eglDisplay, configs[0]!!, inputSurface,
        ...
        fun shader(t: Int, s: String) = GLES20.glCreateShader(t).also { GLES20.glShaderSource(it, s); GLES20.glCompileShader(it) }
        ...
        fun fb(d: FloatArray) = ByteBuffer.allocateDirect(d.size * 4).order(ByteOrder.nativeOrder()).asFloatBuffer().also { it.put(d); it.position(0) }
        val POS = fb(floatArrayOf(-1f,-1f, 1f,-1f, -1f,1f, 1f,1f))
        val UV  = fb(floatArrayOf( 0f, 1f, 1f, 1f,  0f,0f, 1f,0f))
        val aP = GLES20.glGetAttribLocation(prog, "aPos"); val aT = GLES20.glGetAttribLocation(prog, "aTex")
        GLES20.glUseProgram(prog); GLES20.glActiveTexture(GLES20.GL_TEXTURE0)
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, tex[0])
        GLES20.glUniform1i(GLES20.glGetUniformLocation(prog, "uTex"), 0)
        GLES20.glEnableVertexAttribArray(aP); GLES20.glEnableVertexAttribArray(aT)
        GLES20.glVertexAttribPointer(aP, 2, GLES20.GL_FLOAT, false, 0, POS)
        GLES20.glVertexAttribPointer(aT, 2, GLES20.GL_FLOAT, false, 0, UV)
        ...
        fun drain(eos: Boolean) {
            ...
            while (System.currentTimeMillis() < deadline) {
                ...
                when {
                    idx == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED -> { trackIdx = muxer.addTrack(codec.outputFormat); muxer.start(); muxerStarted = true }
                    idx >= 0 -> {
        ...
        for (i in 0 until TOTAL_FRAMES) {
            GLES20.glViewport(0, 0, W, H); GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT)
        ...
        muxer.release(); inputSurface.release()

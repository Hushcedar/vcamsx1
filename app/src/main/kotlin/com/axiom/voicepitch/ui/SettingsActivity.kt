...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        semitoneLabel = TextView(this).apply {
            textSize = 18f
            setTypeface(null, Typeface.BOLD)
            setTextColor(Color.parseColor("#6C63FF"))
            setPadding(0, 0, 0, 8)
        }
        root.addView(semitoneLabel)
        ...
        listOf("Robot" to -12f, "Deep" to -6f, "Normal" to 0f, "High" to 6f, "Chip" to 12f).forEach { (label, st) ->
        ...
        root.addView(TextView(this).apply {
            text = "⚠  Force-stop the target app after changing settings.\nScope this module to target apps in LSPosed Manager."
            textSize = 12f
            setTextColor(Color.parseColor("#888888"))
            lineSpacingMultiplier = 1.5f
        })
        updateLabel(progressToSt(seekBar.progress))
